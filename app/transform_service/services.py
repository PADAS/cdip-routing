...
def get_all_outbound_configs_for_id(
    destinations_cache_db: walrus.Database, inbound_id: UUID
    ...
    try:
        ...
        logger.exception(
            ...
            extra={"inbound_integration_id": inbound_id},
        )
        raise
    ...
    if resp.status_code == 200:
        try:
            ...
            logger.error(
                "Failed decoding response for OutboundConfig for Inbound(%s), response text: %s",
                inbound_id,
                resp.text,
                extra={"inbound_integration_id": inbound_id},
            )
            ...
            configs, errors = schemas.get_validated_objects(
                resp_json, schemas.OutboundConfiguration
            )
            return configs
...
async def ensure_device_integration(integration_id: str, device_id: str):
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            device_data = await portal.ensure_device(
